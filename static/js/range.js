...
importButton.addEventListener("click", function (){
  ...
  //   "ctx": {},
  //   "totalDuration": 10,
  //   "startDuration": 7.617924528301887,
  //   "endDuration": 4.882075471698113,
  //   "currentPosition": 0,
  //   "draggable": true,
  //   "mouseX": 600,
  //   "mouseY": 37,
  ...
  //   currcanvas.setAttribute("id", "Gloss");
  ...
  //   customRangeSlider.init(10, "Gloss");
  //
  ...
  //   addColValue.setAttribute("id", "addValueGloss");
  //   var addName = document.createElement("a");
  //   addName.innerHTML = "Gloss";
...
scissorsButton.addEventListener("click", async function (){
  ...
  // uploadVideo();
  document.getElementById('SigningAreaLabel').innerHTML = "Running."
  await sleep(1000);
  document.getElementById('SigningAreaLabel').innerHTML = "Running.."
  await sleep(1000);
  document.getElementById('SigningAreaLabel').innerHTML = "Running..."
  ...
  document.getElementById('SigningAreaLabel').innerHTML = "Running.."
  await sleep(1000);
  document.getElementById('SigningAreaLabel').innerHTML = "Signing Area"

  if ("Signing" in ranges){
    console.log("Already exists");
    console.log(ranges);
  } else {
    ranges["Signing"] = {
    ...
    var rowNameDiv = document.createElement("div");
    rowNameDiv.setAttribute("id", "Signing_rowname");
    rowNameDiv.setAttribute("style", "padding: 8px; height: 40px; z-index: 0;");
    var rowName = document.createElement("a");
    rowName.innerHTML = "Signing";
    ...
    rowDeleteIcon.addEventListener('click', function(){
      if (confirm('Are you sure you want to remove row ' + "Signing" + "?" )) {
        delete ranges["Signing"];
        console.log(ranges);
        var curcanvas = document.getElementById("Signing");
        document.getElementById('metadata_timeline').removeChild(curcanvas);
        var curname = document.getElementById("Signing" + "_rowname");
        console.log(curname);
        metadata_name.removeChild(curname);
        var additionalName = document.getElementById("addValueName" + "Signing");
        var additionalValue = document.getElementById("addValue" + "Signing");
        var addDiv = document.getElementById('additionalVisualizationRow');
    ...
    var currcanvas = document.createElement("canvas");
    currcanvas.setAttribute("id", "Signing");
    currcanvas.setAttribute("padding-top", "-10px");
    currcanvas.setAttribute("z-index", "0");
    currcanvas.setAttribute("position", "absolute");
    document.getElementById('metadata_timeline').appendChild(currcanvas);
    customRangeSlider.init(10, "Signing");
    //additionalVisualization
    var addColName = document.createElement("div");
    addColName.setAttribute("class", "col-6");
    addColName.setAttribute("style", "height:80px;");
    var addColValue = document.createElement("div");
    addColValue.setAttribute("class", "col-6");
    addColValue.setAttribute("id", "addValueSigning");
    var addName = document.createElement("a");
    addName.innerHTML = "Signing";
    addColName.append(addName);
...
function disableHandshapeButton() {
  ...
  handshapesButton.style.color = 'grey';
...
function activateHandshapeButton(){
  ...
  handshapesButton.style.color = 'black';
...
handshapesButton.addEventListener("click", async function (){
  if (this.style.color == 'grey') {
    ...
    document.getElementById('HandshapeLabel').innerHTML = "Running."
    await sleep(1000);
    document.getElementById('HandshapeLabel').innerHTML = "Running.."
    await sleep(1000);
    document.getElementById('HandshapeLabel').innerHTML = "Running..."
    ...
    document.getElementById('HandshapeLabel').innerHTML = "Running.."
    await sleep(1000);
    document.getElementById('HandshapeLabel').innerHTML = "Right hand"
    var curr_range = getActiveRange();
    // console.log()
    ...
    if ("Right hand" in ranges){
      ...
      currcanvas.setAttribute("position", "absolute");
      document.getElementById('metadata_timeline').appendChild(currcanvas);
      customRangeSlider.init(10, "Right hand");
...
// handshapesButton2.addEventListener("click", function (){
//   if ("Right hand" in ranges){
//     console.log("Already exists");
//   } else {
//     ranges['Right hand'] = {
...
//             ],
//             "type": "handshapes",
//             "value": "http://slan-tool.com/media/fourspread20.png",
//             "id": "hPoiM9"
//         },
//         {
//             "range_pix": [
...
//             ],
//             "type": "handshapes",
//             "value": "http://slan-tool.com/media/hand5.png",
//             "id": "plG84q"
//         },
//         {
//             "range_pix": [
...
//             ],
//             "type": "handshapes",
//             "value": "http://slan-tool.com/media/twofingersnonspread6.png",
//             "id": "eq1WNc"
//         },
//         {
//             "range_pix": [
...
var customRangeSlider = {
    ...
    init: function(totalDuration, canvas_id) {
          ...
          console.log("Keys currently pressed:" + keysArray);
            if(lastDownTarget ==rs['canvas']) {
              if (event.key === "a") {
                ...
                var id = makeid(6);
                rs['ranges'].push({'range_pix' : [startpoint, startpoint+50],
                  'range': [startpoint_per, endpoint_per], "type": "null", "value": "", "value_icon":"","id": id});
                active = id;
        ...
        rs.canvas.addEventListener("dblclick", function openValue(){
          for (var i=0; i < rs['ranges'].length; i++){
            //RangeStart
            if (event.layerX > rs['ranges'][i]['range'][0]*rs['canvas'].getBoundingClientRect().width && event.layerX < rs['ranges'][i]['range'][1]*rs['canvas'].getBoundingClientRect().width) {
              ...
              // rs.canvas.popover.show();
              // $('#'+ canvas_id).popover('show');
            }
          }
...
function addRowFunction(){
  ...
  var rowNameDiv = document.createElement("div");
  rowNameDiv.setAttribute("style", "padding: 8px; height: 40px; z-index: 0;");
  rowNameDiv.setAttribute("id", row_name.value + "_rowname");
  ...
  rowDeleteIcon.setAttribute("class", "bi bi-x deleteRow");
  rowDeleteIcon.addEventListener('click', function(){
    if (confirm('Are you sure you want to remove row ' + newRowName + "?" )) {
      delete ranges[newRowName];
      console.log(ranges);
      var curcanvas = document.getElementById(newRowName);
      document.getElementById('metadata_timeline').removeChild(curcanvas);
      var curname = document.getElementById(newRowName + "_rowname");
      console.log(curname);
      metadata_name.removeChild(curname);
      var additionalName = document.getElementById("addValueName" + newRowName);
      var additionalValue = document.getElementById("addValue" + newRowName);
      var addDiv = document.getElementById('additionalVisualizationRow');
      addDiv.removeChild(additionalName);
      addDiv.removeChild(additionalValue);
  ...
  metadata_name.appendChild(rowNameDiv);
  var currcanvas = document.createElement("canvas");
  ...
  row_name.value = "";
  document.getElementById('metadata_timeline').appendChild(currcanvas);
  customRangeSlider.init(10, rowName.innerHTML);
...
document.getElementById('row_name').addEventListener('keyup', function(){
  if (event.keyCode === 13) {
        addRowFunction();
    }
}, false);
...
function drawSubprogressBar(){
  ...
  metadata_pointer.height = 1000;
  metadata_pointer_ctx.beginPath();
  metadata_pointer_ctx.moveTo(0, 0);
  metadata_pointer_ctx.lineTo(0, 1000);
...
video.addEventListener('timeupdate', function(event){
  ...
  metadata_pointer_ctx.lineTo(subpointer_padding+5, 1000);
  metadata_pointer_ctx.strokeStyle = "rgb(42,148,255)";
  metadata_pointer_ctx.lineWidth = 1;
  metadata_pointer_ctx.stroke();
...
trimVideoButton.addEventListener('click', async function(){
  ...
  if ("Left hand" in ranges){
    ...
    currcanvas.setAttribute("position", "absolute");
    document.getElementById('metadata_timeline').appendChild(currcanvas);
    customRangeSlider.init(10, "Left hand");
